...
impl<const NRES: usize, const NTH: usize> Resource<NRES, NTH>{
    ...
    fn take(&mut self, id: usize, resouce: usize) -> bool {
        if id >= NTH || resouce >= NRES || self.available[resouce] == 0 || self.max[id][resouce] == self.allocation[id][resouce] {
            return false;
        }

        self.allocation[id][resouce] += 1;
        self.available[resouce] -= 1;
    ...
    fn release(&mut self, id: usize, resouce: usize){
        if id >= NTH || resouce >= NRES || self.allocation[id][resouce] == 0 {
            return;
        }
...
impl<const NRES: usize, const NTH: usize> Banker<NRES, NTH>{
    ...
    pub fn take(&self, id: usize, resouce: usize) -> bool {
        let mut r = self.resource.lock().unwrap();
        r.take(id, resouce)
    }

    pub fn release(&self, id: usize, resouce: usize){
        let mut r = self.resource.lock().unwrap();
        r.release(id, resouce)
    }
}